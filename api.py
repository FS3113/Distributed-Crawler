...
def get_department_list():
    mydb, mycursor = connect_to_mysql()
    if len(request.json) == 0 or request.json == 'All':
        return {'department': []}
    q = request.json.split('. ')
...
def submit_query():
    ...
    if len(request.json) == 0:
        return {'result': []}
    if len(request.json['department']) > 0 and request.json['department'] != 'All':
        university_id = int(request.json['university'].split('. ')[0])
...
def get_task():
    mydb, mycursor = connect_to_mysql()
    if request.json['university'] == 'ALL':
...
def get_detailed_statistics_of_university():
    ...
    if len(request.json) == 0:
        return {'department': []}
    q = request.json['university'].split('. ')
...
def get_next_tasks():
    ...
    for i in range(50 * (num - 1), min(50 * num, len(r))):
        ...
        res.append([str(r[i][0]) + '. ' + university, r[i][1]])
    mycursor.close()
    mydb.close()
    return {'res': res}
...
def prioritize_tasks():
    ...
    mycursor.execute('select min(Priority) from Faculty_Tasks;')
    n = mycursor.fetchone()[0]
    for i in table[::-1]:
        if len(i) == 3 and i[-1]:
            n -= 1
            id = i[0][:i[0].find('.')]
            mycursor.execute('update Faculty_Tasks set Priority = {} where University_ID = "{}" and Department_Name = "{}"'.format(n - 1, id, i[1]))
    mydb.commit()
    mycursor.close()
    mydb.close()
    return {}
